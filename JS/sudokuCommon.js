...
class ClockedFiller {
    constructor() {
        this.timer = false;
        this.storeIsFilled = false;
    }

    async start() {
        if (!this.isRunning()) {
            this.timer = window.setInterval(async () => {
                this.storeIsFilled = await sudoApp.mySolver.fillStepInWebWorkerPuzzleStore();
                if (this.storeIsFilled) {
                    this.stop();
                }
            }, 500);
        }
...
class SudokuSolver extends MVC_Model {
    ...
    constructor(app) {
        ...
        // this.myClockedFiller = new ClockedFiller();
    ...
    init() {
        ...
        this.fillPuzzleStore();
        // this.myClockedFiller.start();
    }
    ...
    async fillPuzzleStore() {
        let isFilled = await this.fillStepInWebWorkerPuzzleStore();
        while (!isFilled) {
            // sudoApp.mySolverController.sleep(500);
            isFilled = await this.fillStepInWebWorkerPuzzleStore();
        }
    }
    
    async fillStepInWebWorkerPuzzleStore() {
        let myFillStep = () => new Promise(function (myResolve, myReject) {
            ...
            // Post request
            let request = {
                name: 'fillPuzzleStore',
                value: ''
            }
            let str_request = JSON.stringify(request);
            sudoApp.mySolver.webworkerGenerator.postMessage(str_request, [channel.port2]);
        });
        let str_response = await myFillStep();
        let response = JSON.parse(str_response);
        let isFilled = response.value;
        return isFilled;
    }



    async getNewPuzzleFromWebWorker(level) {
...
class SudokuSolverController {
    ...
    async generateLinkPressed(level) {
        ...
        let puzzleRecord = await sudoApp.mySolver.getNewPuzzleFromWebWorker(level);
