...
class NewPuzzleBuffer {
    ...
    constructor() {
        ...
        this.heavyPushPromise = undefined;
    ...
    async verySimplePuzzlePushed() {
        return new Promise(function (resolve, reject) {
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.addEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.verySimplePushWaitingListener = function (event) {
    ...
    async simplePuzzlePushed() {
        return new Promise(function (resolve, reject) {
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.addEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.simplePushWaitingListener = function (event) {
                    let request = JSON.parse(event.data);
                    if (request.name == 'puzzleGenerated') {
                        let puzzleRecord = request.value;
    ...
    async mediumPuzzlePushed() {
        return new Promise(function (resolve, reject) {
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.addEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.mediumPushWaitingListener = function (event) {
                    let request = JSON.parse(event.data);
                    if (request.name == 'puzzleGenerated') {
                        let puzzleRecord = request.value;
                        if (puzzleRecord.preRunRecord.level == 'Mittel') {
                            resolve(true);
                        }
                    }
                },
    ...
    async heavyPuzzlePushed() {
        return new Promise(function (resolve, reject) {
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.addEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.heavyPushWaitingListener = function (event) {
                    let request = JSON.parse(event.data);
                    if (request.name == 'puzzleGenerated') {
                        let puzzleRecord = request.value;
                        if (puzzleRecord.preRunRecord.level == 'Schwer') {
                            resolve(true);
                        }
                    }
                },
    ...
    async veryHeavyPuzzlePushed() {
        return new Promise(function (resolve, reject) {
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.addEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.veryHeavyPushWaitingListener = function (event) {
                    let request = JSON.parse(event.data);
                    if (request.name == 'puzzleGenerated') {
                        let puzzleRecord = request.value;
                        if (puzzleRecord.preRunRecord.level == 'Sehr schwer') {
                            resolve(true);
                        }
                    }
                },
    ...
    async extremeHeavyPuzzlePushed() {
        return new Promise(function (resolve, reject) {
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.addEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.extremeHeavyPushWaitingListener = function (event) {
                    let request = JSON.parse(event.data);
                    if (request.name == 'puzzleGenerated') {
                        let puzzleRecord = request.value;
                        if (puzzleRecord.preRunRecord.level == 'Extrem schwer') {
                            resolve(true);
                        }
                    }
                },
    ...
    async getVerySimplePuzzle() {
        ...
        if (this.verySimplePuzzles.length > 0) {
            ...
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.removeEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.verySimplePushWaitingListener,
                false
            );
        }
        if (!this.isFilled()) {
    ...
    async getMediumPuzzle() {
        ...
        if (this.mediumPuzzles.length > 0) {
            ...
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.removeEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.mediumPushWaitingListener,
                false
            );
        }
        if (!this.isFilled()) {
    ...
    async getHeavyPuzzle() {
        ...
        if (this.heavyPuzzles.length > 0) {
            ...
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.removeEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.heavyPushWaitingListener,
                false
            );
        }
        if (!this.isFilled()) {
    ...
    async getVeryHeavyPuzzle() {
        ...
        if (this.veryHeavyPuzzles.length > 0) {
            ...
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.removeEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.veryHeavyPushWaitingListener,
                false
            );
        }
        if (!this.isFilled()) {
    ...
    async getExtremeHeavyPuzzle() {
        ...
        if (this.extremeHeavyPuzzles.length > 0) {
            ...
            sudoApp.myNewPuzzleBuffer.webworkerGenerator.removeEventListener(
                'message',
                sudoApp.myNewPuzzleBuffer.extremeHeavyPushWaitingListener,
                false
            );
        }
        if (!this.isFilled()) {
    ...
    pushMsgHandler(event) {
        let request = JSON.parse(event.data);
        if (request.name == 'puzzleGenerated') {
            try {
                let puzzleRecord = request.value;
                sudoApp.myNewPuzzleBuffer.pushPuzzle(puzzleRecord);
...
class SudokuSolverController {
    ...
    async generateLinkPressed(level) {
        ...
        switch (level) {
            case 'Sehr leicht': {
                puzzleRecord = await sudoApp.myNewPuzzleBuffer.getVerySimplePuzzle();
                break;
            }
            case 'Leicht': {
                puzzleRecord = await sudoApp.myNewPuzzleBuffer.getSimplePuzzle();
                break;
            }
            case 'Mittel': {
                puzzleRecord = await sudoApp.myNewPuzzleBuffer.getMediumPuzzle();
                break;
            }
            case 'Schwer': {
                puzzleRecord = await sudoApp.myNewPuzzleBuffer.getHeavyPuzzle();
                break;
            }
            case 'Sehr schwer': {
                puzzleRecord = await sudoApp.myNewPuzzleBuffer.getVeryHeavyPuzzle();
                break;
            }
            case 'Extrem schwer': {
                puzzleRecord = await sudoApp.myNewPuzzleBuffer.getExtremeHeavyPuzzle();
                break;
            }
            default: {
                throw new Error('Unexpected difficulty: '
